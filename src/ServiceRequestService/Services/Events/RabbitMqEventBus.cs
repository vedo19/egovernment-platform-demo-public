    ...
    private void EnsureChannel()
    {
        ...
        lock (_connectLock)
        {
            ...
            try
            {
                ...
                _connection = factory.CreateConnection("service-request-service-publisher");
                _channel = _connection.CreateModel();
                _channel.ExchangeDeclare(ExchangeName, ExchangeType.Topic, durable: true, autoDelete: false);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to establish RabbitMQ connection from ServiceRequestService");
                _channel = null;
            }
        }
    }
